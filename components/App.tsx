...
import SoftwareTrial from './SoftwareTrial';
...
import { X, ArrowRight, Calendar, Clock, Search, Hash, Terminal, Radio } from 'lucide-react';
...
const CommandPalette: React.FC<{ isOpen: boolean; onClose: () => void; onNavigate: (id: string) => void }> = ({ isOpen, onClose, onNavigate }) => {
    ...
    const commands = [
        { id: 'work', label: 'Access Deployments', icon: <Hash size={14} />, action: () => onNavigate('work') },
        { id: 'process', label: 'View Protocol', icon: <Hash size={14} />, action: () => onNavigate('process') },
        { id: 'pricing', label: 'Clearance Levels', icon: <Hash size={14} />, action: () => onNavigate('pricing') },
        { id: 'blog', label: 'System Intelligence', icon: <Hash size={14} />, action: () => onNavigate('blog') },
        { id: 'contact', label: 'Initiate Uplink', icon: <Radio size={14} />, action: () => onNavigate('contact') },
    ];
    ...
    return (
        <div className="fixed inset-0 z-[200] flex items-start justify-center pt-[20vh] animate-in fade-in duration-200">
            ...
            <div className="relative w-full max-w-lg bg-[#151515] border border-white/20 rounded-xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
                ...
                <div className="p-2">
                    <div className="text-[10px] font-mono uppercase text-gray-600 px-3 py-2">System Commands</div>
                    {commands.map((cmd) => (
                        <button 
                            key={cmd.id}
...
const App: React.FC = () => {
  ...
  return (
    <>
      ...
      <div className={`bg-black min-h-screen text-white relative transition-opacity duration-1000 ${isLoading ? 'opacity-0' : 'opacity-100'}`}>
        ...
        <nav className="fixed top-0 left-0 w-full z-50 px-6 py-4 flex justify-between items-center bg-[#0d0d0d]/80 backdrop-blur-md border-b border-white/5">
             ...
             <div className="hidden md:flex items-center gap-8 font-mono text-xs text-gray-400">
                <button onClick={() => scrollToSection('process')} className="hover:text-white transition-colors hover:underline decoration-blue-500 decoration-2 underline-offset-4 font-bold tracking-widest">PROTOCOL</button>
                <button onClick={() => scrollToSection('work')} className="hover:text-white transition-colors hover:underline decoration-blue-500 decoration-2 underline-offset-4 font-bold tracking-widest">DEPLOYMENTS</button>
                <button onClick={() => scrollToSection('software')} className="hover:text-white transition-colors hover:underline decoration-blue-500 decoration-2 underline-offset-4 font-bold tracking-widest">LAB</button>
                <button onClick={() => scrollToSection('pricing')} className="hover:text-white transition-colors hover:underline decoration-blue-500 decoration-2 underline-offset-4 font-bold tracking-widest">ACCESS</button>
             </div>
        ...
        <Overlay 
            ...
            title={activePost?.title || 'System Log'}
        >
            {activePost && (
